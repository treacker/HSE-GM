...
def calculate_activation_statistics(dataloader, model, classifier):
    ...
    with torch.no_grad():
        for image, _ in dataloader:
            ...
            activs = classifier.get_activations(image).reshape((image.shape[0], -1))
            recon_activs = classifier.get_activations(recon_image).reshape((image.shape[0], -1))
            original = torch.cat((original, activs), 0)
            reconstructed = torch.cat((reconstructed, recon_activs), 0)

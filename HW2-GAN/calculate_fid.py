...
def calculate_activation_statistics(dataloader, model, classifier, labels_idx):
    classifier.eval()
    model.eval()
    device = model.device
    ...
    with torch.no_grad():
        for image, label in dataloader:
            ...
            new_image = model.generate(image, new_label)
...
@torch.no_grad()
def calculate_fid(dataloader, model, classifier, labels_idx):
    
    m1, s1, m2, s2 = calculate_activation_statistics(dataloader, model, classifier, labels_idx)
